...
from sophia.db.service import create_user_file, get_user_images
...
@router.get("/api/images", response_model=list[ImageGenerateOut])
def list_images_endpoint(
    user: User = Depends(get_authenticated_user),
    session: Session = Depends(get_db_session),
) -> list[ImageGenerateOut]:
    """
    Executive Brief:
        Return all images generated by the authenticated user, newest first.
        Each entry carries id, filename, mime type, and a /api/files/{id}/raw URL.
        Unauthenticated requests are rejected by the dependency (401).
        Returns an empty list when the user has no images.
    """
    image_records = get_user_images(session, user.id)
    return [
        ImageGenerateOut(
            id=record.id,
