...
def get_user_images(session: Session, user_id: int) -> list[UserFile]:
    """
    Executive Brief:
        Return all image files generated by the given user, newest first.
        Owner-scoped so one user can never see another's images.
    ...
    Returns:
        list[UserFile]: Image records ordered by created_at descending.
                        Returns an empty list when the user has no images.

    Scale Note:
        O(n) where n = number of images for this user. For typical usage
        (hundreds of images), a single query is sufficient. Add pagination
        only when a user exceeds ~10k images.
    """
    return (
        session.query(UserFile)
        .filter(
