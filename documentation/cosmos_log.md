...
The database schema is now under version control. Until today the tables were
born from `Base.metadata.create_all` — fine for a fresh database, useless the
moment the schema needs to change without dropping everything. Alembic fixes
that. Scaffolded `alembic/` with `alembic init`, pointed `alembic/env.py` at the
single `Base.metadata` from `sophia/db/models.py`, and made it read the same
`SOPHIA_DB_URL` the app reads, so migrations and the running app can never
target different databases. One autogenerated initial migration creates users,
conversations, and messages.

**The thing that surprised me:** autogenerate does not diff against the models
alone — it diffs the models against whatever database it connects to. My dev
`sophia_memory.db` already held all three tables, so the first autogenerate
produced an empty migration. The fix was to generate against a fresh throwaway
database (`SOPHIA_DB_URL` pointed at a temp file), so Alembic saw a blank slate
and emitted the full set of `create_table` calls. Then I deleted the scratch
file.

**The thing that taught me something:** the live dev database had the tables but
no `alembic_version` row, so a plain `upgrade head` would have errored trying to
recreate tables that already exist. `alembic stamp head` is the right tool — it
records the database as already migrated without running any DDL, which is how
you adopt Alembic on an existing schema without losing data. Afterwards
`alembic check` reports "No new upgrade operations detected" — proof the
migration matches the models exactly.

**One decision for the future:** enabled `render_as_batch=True` in env.py now.
SQLite cannot `ALTER TABLE ... DROP COLUMN` directly; batch mode rebuilds the
table behind the scenes. Costs nothing today and saves the first column change
from breaking later.

**Testing:** `tests/test_migrations.py` drives the real migration scripts
against a throwaway temp database — `upgrade head` must create exactly the three
tables, `downgrade base` must leave none. A subtle isolation trap surfaced here:
env.py overwrites the Config URL from `SOPHIA_DB_URL` at run time, so the test
sets that env var to its own temp path; otherwise the migration would run
against the real dev database. Suite now 146 tests, all passing.

**Next step:** Phase 14 — Testing. Round out the suite: retriever, orchestrator
prompt injection, the auth flow, and a full `/api/chat` request.
